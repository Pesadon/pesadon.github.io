// ===== SHARED EVENTS DATA =====
// Ported from esemenyek.js, unchanged, just turned into an ES module so
// pages can `import` exactly what they need instead of relying on a global
// <script> tag. Phase 3 of the migration will replace the EVENTS array
// below with a fetch from the D1 database — every function below will keep
// working exactly as it does now, since nothing that reads this data needs
// to change.
//
...
export function formatDateRange(ev) {
  const s = ev._start;
  const e = ev._end;
...
export function eventCoversDate(ev, year, month, day) {
...
export const MONTHS_HU_GEN = [
  "január","február","március","április","május","június",
  "július","augusztus","szeptember","október","november","december"
];

export const DAYS_HU_LONG = ["vasárnap","hétfő","kedd","szerda","csütörtök","péntek","szombat"];